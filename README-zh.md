...
- **从语音起点开始裁**。起点前混入的呼吸声、尾音会被学成"句首怎么起音"的模板，于是逐句复现成开场杂音。这是第 5 步里第 1 条成因的防线——这一步切干净，就把那一类杂音排除掉了
...
合成片段的开头可能出现一声"啊"。它不一定出现；真出现了，成因也不止一种，处理方式完全不同：

| 成因 | 怎么判断 | 怎么处理 |
|---|---|---|
| **参考音频起点脏**：起点前混入呼吸声、尾音或上一句残余，被 zero-shot 学成"句首怎么起音"的模板，于是逐句复现 | 每段开头的杂音**高度一致**，且能在参考音频里找到对应那段声音 | 回第 2 步重裁，严格从语音起点开始，能根治 |
| **生成侧起音**：LLM 首个语音 token 自带元音起音 | 参考音频已切干净，杂音**时有时无**、力度不一 | 改不掉，只能后处理切除 |

第 1 条有架构依据（见 [DESIGN.md](DESIGN.md) §8：参考音频的三路条件注入，任一路脏了成品都会复现）；第 2 条**尚无对照实验证实**，§5 里记了原因。所以别急着给自己定责，按下面的判断依据走。

**先抽听两三个 `seg_XX.wav`**：开头干净就跳过本步；有杂音再往下跑。脚本检不出可切结构、或切割量超过 0.8s 保护上限时会原样保留，不会误切。

```bash
.venv/bin/python scripts/10_trim_onset.py audio/raw_run1 audio/raw_run1_trim
```

输出目录里的片段直接用于后续步骤。若切完每段开头仍有残留，回到上表按判断依据确认成因——尤其要确认参考音频起点是不是真的干净，那是唯一能根治的一条。
...
4. **每句开头有"啊"**：成因见第 5 步——参考音频起点脏，或生成侧起音。先按表里的判断依据分清是哪一类：前者回第 2 步重裁参考音频，后者跑切除脚本。
